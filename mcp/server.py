...
from app.file_system import FolderContents, FileMetadata, SizeLimitKind, SnapshotList
...
@mcp.tool()
def folder_contents(path: str = "", snapshot_id: str | None = None) -> FolderContents:
    """
    ...
    Optional snapshot_id parameter allows browsing a historical snapshot (e.g., from .snapshot).
    """

    return file_system_client.folder_contents(path, snapshot_id=snapshot_id)

@mcp.tool()
def file_metadata(path: str, snapshot_id: str | None = None) -> FileMetadata:
    """
    ...
    The path is relative to the root folder. Names are delimited with '/'.
    Optional snapshot_id parameter allows retrieving metadata from a snapshot.
    """
    return file_system_client.get_metadata(path, snapshot_id=snapshot_id)


@mcp.tool()
def file_contents(path: str, snapshot_id: str | None = None) -> str:
    """
    ...
    The path is relative to the root folder. Names are delimited with '/'.
    Optional snapshot_id parameter allows retrieving the contents from a snapshot.
    """
    return file_system_client.get_file_content_as_string(path, snapshot_id=snapshot_id)

@mcp.tool()
def file_contents_base64(path: str, snapshot_id: str | None = None) -> str:
    """
    ...
    Optional snapshot_id parameter allows retrieving the contents from a snapshot.
    """
    contents = file_system_client.get_file_content(path, snapshot_id=snapshot_id)
    encoded_contents = base64.b64encode(contents).decode("utf-8")
    return encoded_contents
    
@mcp.tool()
def image_file_contents(path: str, thumb_width: int = 0, snapshot_id: str | None = None) -> Image:
    """
    ...
    image_data = file_system_client.get_file_content(
        path,
...
@mcp.tool()
def file_file_contents_as_text(path: str, snapshot_id: str | None = None) -> str:
    """
    ...
    text = extract_text_from_file(path, 
                                  file_system_client.get_file_content(
                                      path,
                                      size_limit_kind=SizeLimitKind.READ,
                                      snapshot_id=snapshot_id,
                                  )
                                  )
...
@mcp.tool()
def list_snapshots(path: str = "") -> SnapshotList:
    """
    List available snapshot directories for the configured path.
    The path is relative to the root folder. Names are delimited with '/'.
    """
    return file_system_client.list_snapshots(path)
